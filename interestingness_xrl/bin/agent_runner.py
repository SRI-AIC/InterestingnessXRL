...
def video_schedule(e):
    # linear capture schedule
    return args.videos and \
           (e == config.num_episodes - 1 or e % int(config.num_episodes / config.num_recorded_videos) == 0)


def run_trial(config, num_trial):
    # creates env helper
    helper = create_helper(config)

    # checks for provided output dir
    output_dir = args.output if args.output is not None else get_agent_output_dir(config, agent_t, num_trial)
    ...
    env_id = '{}-{}-v0'.format(config.gym_env_id, num_trial)
    helper.register_gym_environment(env_id, False, FPS, SHOW_SCORE_BAR)

    # create environment and monitor
    env = gym.make(env_id)
    env = Monitor(env, directory=output_dir, force=True, video_callable=video_schedule)
    ...
    env.seed(config.seed + num_trial)
    agent_rng = np.random.RandomState(config.seed + num_trial)
    ...
    for e in range(config.num_episodes):
        ...
        env.env.monitor = env if video_schedule(e) else None
